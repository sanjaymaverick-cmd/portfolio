"""Vega defense — pure functions from docs/v2/VEGA_HEDGE_SKETCH.md."""
...
def aggregate_vega(
    ...
    return VegaExposure(
        ...
        stale=any(x.stale for x in use),
    )
...
def evaluate_vega(
    ...
    *,
    ...
    hvp = pol.hedge_vega_per_lot if pol.hedge_vega_per_lot is not None else hedge_vega_per_lot
    ...
    raw = option_lots_to_hit_vega_target(exp.net_vega, target, hvp) if need_trade else 0.0
    actionable = snap_lots(raw, pol.lot_step)
    if exp.stale:
        actionable = 0.0
        raw = 0.0
    residual = exp.net_vega + actionable * hvp
    fut = futures_lots_to_flatten_delta(exp.net_delta_lots, actionable, hd) if actionable else 0.0
    ...
    copy = build_vega_copy(exp.symbol, exp, pol, actionable, fut, util) if urgency != "none" else ""
    if exp.stale and copy:
        copy += " Greeks look stale — actionable lots muted until a fresh mark."
    ...
    return VegaReview(
        ...
        urgency=urgency,
        reason=f"util={util:.2f}; over={over}; stress={regime_stress}; stale={exp.stale}",
        copy_review=copy,
        suppress_reason=None,
    )
