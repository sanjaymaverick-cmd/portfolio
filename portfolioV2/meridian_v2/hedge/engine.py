"""Inventory hedge — pure functions from docs/v2/HEDGE_REBALANCE_SKETCH.md."""
...
class HedgeReview:
    ...
    policy_kind: str = "inventory_hedge"
...
def evaluate_symbol(
    ...
    *,
    ...
    gamma_flag = (
        symbol_policy.gamma_warn_abs > 0 and abs(exposure.net_gamma) >= symbol_policy.gamma_warn_abs
    )
    ...
    if not force:
        ...
        if days_since_last_prompt is not None and days_since_last_prompt < policy.min_days_between_prompts:
    ...
    return HedgeReview(
        ...
        reason=(
            f"h_star={h_star:.2f}; drift_lots={drift:.3f}; breached={breached}; "
            f"regime={regime.value}; net_gamma={exposure.net_gamma:.6f}; gamma_flag={gamma_flag}"
        ),
        copy_review=copy,
